...
async function updateCommand(domain: string) {
  ...
  if (wallet && !walletSig) {
    ...
    console.error('')
    console.error('You must prove you own the wallet by signing your domain name.')
    console.error('Use the bundled signing tool (your key is used locally, never transmitted):')
    console.error('')
    console.error(`  qz-sign-domain ${domain} --key <your-private-key>`)
    console.error(`  # or: QZ_WALLET_KEY=0x... qz-sign-domain ${domain}`)
    console.error('')
    console.error('Or sign with any tool you trust (MetaMask, Ledger, cast, ethers):')
    console.error(`  # MetaMask (browser console, key never leaves extension):`)
